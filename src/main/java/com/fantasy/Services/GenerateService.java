...
public class GenerateService {
    ...
    public void generate(int gameWeek) {
        ...
        for (Game g : gw.getGames()) {
            ...
            while (golosEquipa1 > 0) {
                ...
                golosEquipa1--;
                String eventType = "";
                switch (marcador.getPosition()){
                    case "GK": eventType = "GOAL GK"; break;
                    case "DEF": eventType = "GOAL DEF"; break;
                    case "MID": eventType = "GOAL MID"; break;
                    case "FOR": eventType = "GOAL FOR"; break;
                }
                GameEvent gameEvent = new GameEvent(eventType, r.nextInt(90) + 1, g, marcador);
                eventosJogo.add(gameEvent);
            }
    ...
    public void populateRealTeamsPlayers() throws Exception {
        ...
        List<EventPoints> events = new ArrayList<>();
        events.add(new EventPoints("YELLOW CARD",-3));
        events.add(new EventPoints("RED CARD",-5));
        events.add(new EventPoints("GOAL GK",99999999));
        events.add(new EventPoints("GOAL DEF",5));
        events.add(new EventPoints("GOAL MID",4));
        events.add(new EventPoints("GOAL FOR",3));
...
>>>>>>> 08621adc5710512835b626605fd1ae5cb331cdc3
    ...
    public void calculatePointsForSnapshots(GameWeek gw, List<GameEvent> gameEvents,HashMap<String,Integer> pointMap) {
        ...
        for(VirtualTeam vt: allTeams){
            ...
            for(GameEvent e: gameEvents){
                Player playerFromEvent = e.getPlayer();
                //Se player em causa no evento pertence à equipa
                if(playerHash.containsKey(playerFromEvent.getName())){
                    int mult = 1;
                    if(snap.getCapitao() == playerFromEvent.getId())
                        mult = 2;
                    snap.addPointsFromEvent(pointMap.get(e.getType()) * mult);
                }
            }
            //QUANDO JA TEM OS PONTOS TODOS SOMADOS
            vt.setTotalPoints(snap.getGameWeekCumulativePoints());
            virtualTeamDAO.save(vt);
            snapDAO.save(snap);
        }
    }
